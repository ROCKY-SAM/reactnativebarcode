  ...
  const handleBarCodeScanned = ({ type, data }) => {
    ...
    Alert.alert('Confirm Reading', `Scanned Value Is ${data} Do you wanna save this`, [
      ...
      },
      {
        ...
        style: 'destructive',
        onPress: () => {
          dispatch(barcodesAction.addBarCode(data, 'test', 'test', 'test'));
          setScanned(false);
        }
      }
    ]);
  ...
  return (<ScrollView style={{ flex: 1, backgroundColor: Colors.primary }}>
      ...
      <View>
        {barcodelist.length > 0 ?
          <TouchableOpacity
            onPress={() => { setScanned(true); props.navigation.navigate('ProcessScreen'); }}>
...
AssetReaderScreen.navigationOptions = navData => {
  const from = navData.navigation.getParam('from');
  if (from == "main") {
    return {
      headerTitle: 'Asset Reader'
    };
  } else {
    return {
      headerTitle: 'Asset Reader',
...
export default withNavigationFocus(AssetReaderScreen);
